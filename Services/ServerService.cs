...
public class ServerService : Service, IDisposable
{
    ...
    public override void OnCreate()
    {
        ...
        _enabled = bool.Parse(Task.Run(async () => await SecureStorage.GetAsync("RTSP_SERVER_ENABLED")).Result ?? "True");
        if (!_enabled)
            return;
        _port = Convert.ToInt32(Task.Run(async () => await SecureStorage.GetAsync("RTSP_SERVER_PORT")).Result ?? "7778");
        _maxClients = Convert.ToInt32(Task.Run(async () => await SecureStorage.GetAsync("RTSP_SERVER_MAX_CLIENTS")).Result ?? "7778");
        IPEndPoint endpoint = new(IPAddress.Parse("0.0.0.0"), _port);
...
#pragma warning disable CS8602
        var notification = new NotificationCompat.Builder(this, "foreground_service_channel")
            .SetContentTitle("Server RTSP service Back")
            .SetContentText("Listening other devices")
            .SetSmallIcon(iconId)
            .Build();
